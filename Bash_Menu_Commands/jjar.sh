...
trap "bash ~/Bash_Files/Bash_Menu_Commands/jjar.sh" WINCH
...
choose_from_menu "Please make a choice:" selected_choice "${sorted_selections[@]}"; SSOUND;  
...
if [  "$selected_choice" == "Go Back" ]
  ...
  else cd && cd ~/Java_Files && jar cf $file $fileclass & bash ~/Bash_Files/Bash_Animations/load3.sh
fi
