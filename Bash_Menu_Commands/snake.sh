...
food_color="\e[33;31m" # old \e[34;44m
text_color="\e[31;43m"
no_color="\e[0m"
highscore=0 #marker
...
draw_board() {
    move_and_draw 1 | printf "$border_color\u250C$no_color"
    for ((i=2; i<=width+1; i++)); do
        move_and_draw 1 $i "$border_color─$no_color"
    done
    move_and_draw 1 $((width + 2)) | printf "$border_color\u2510$no_color"
    echo

    for ((i=0; i<height; i++)); do
        move_and_draw $((i+2)) 1 | printf "$border_color\u2502$no_color"
        eval echo -en "\"\${arr$i[*]}\""
        printf "$border_color\u2502$no_color"
    done

    move_and_draw $((height+2)) | printf "$border_color\u2514$no_color"
    for ((i=2; i<=width+1; i++)); do
        move_and_draw $((height+2)) $i "$border_color─$no_color"
    done
    move_and_draw $((height+2)) $((width + 2)) | printf "$border_color\u2518$no_color"
    echo
}
...
sleep 0.25;
    while [ "$alive" -eq 0 ]; do
        echo -e "\n${text_color}  wasd controls q to quit  Your score: $score  Highscore: $highscore $no_color"
    ...
    if [[ $score -ge $highscore ]]; then cd ~/Bash_Files/Bash_Menu_Commands && sed -i 's/highscore=0 #marker/highscore='$score' #marker/g' snake.sh
    fi
