...
Header="TEXT EDITOR SELECTION MENU"
 Footer="ENTER - SELECT,NEXT"
 M0string="Sublimetext"
 M1string="IntellJ"
 M2string="Nano"
 M3string="Go Back"
 M4string="Exit"
...
SSOUND(){ mpv ~/Bash_Files/Bash_Sounds/SelectSound.wav --no-terminal ;} #Custom
 ...
 UNMARK(){ $e "\e[27m";} # normalize current line text
   ...
   EXIT(){ if [ ps -ef | grep "nano" ]; then pgrep -f "nano" | xargs kill
   	   elif [ ps -ef | grep "subl" ]; then pgrep -f "subl" | xargs kill
   	   elif [ ps -ef | grep "flatpak run com.jetbrains.IntelliJ-IDEA-Community" ]; then pgrep -f "flatpak run com.jetbrains.IntelliJ-IDEA-Community" | xargs kill
   	   fi
   exit;}
SCREEN () {
   clear
...
for (( x = 1; x <= $LINES; x++ )); do
  ...
  while [[ "$O" != " " ]]; do case $i in
        0) S=M0;CSOUND & SC;if [[ $cur == "" ]];then SSOUND;R;subl;ES;fi;;
        1) S=M1;CSOUND & SC;if [[ $cur == "" ]];then SSOUND;R;flatpak run com.jetbrains.IntelliJ-IDEA-Community ;ES;fi;;
        2) S=M2;CSOUND & SC;if [[ $cur == "" ]];then SSOUND;R;nano;ES;fi;;
        3) S=M3;CSOUND & SC;if [[ $cur == "" ]];then SSOUND;R;bash ~/Bash_Files/Jcompile_Menu.sh;ES;fi;;
        4) S=M4;CSOUND & SC;if [[ $cur == "" ]];then SSOUND;R;exit;fi;;
  esac;POS;done
